...
describe("test-schema-uid task", function () {
  it("should run with no file and output example UID and Match: true", function () {
    ...
    expect(output).to.include("Using default example schema");
  ...
  it("should run with --file (non-deployed schema) and output calculated UID", function () {
    const schemaPath = path.join(__dirname, "fixtures", "sample-uid.eas.json");
  ...
  it("should exit with error when --file path does not exist", function () {
    const error = runHardhatTask(TASK_NAME, "--file nonexistent/file.eas.json", { expectError: true });
  ...
  it("should exit with error when deployed file has no matching base schema file", function () {
    const deployedPath = path.join("test", "fixtures", "Orphan.deployed.eastest.json");

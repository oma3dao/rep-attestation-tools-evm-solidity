...
import { BAS_SCHEMA_REGISTRY_ADDRESSES } from "../hardhat.config";
import { getProviderAndSigner } from "../utils/provider";
import { ZERO_ADDRESS } from "../utils/constants";
import { calculateSchemaUID, verifySchemaExists, formatSchemaUID, getSchemaDetails } from "../utils/basTools";
...
task("deploy-bas-schema", "Deploy a BAS schema from a .bas.json file")
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    const basSchema: BasSchemaObject = JSON.parse(fileContent);
    ...
    if (hre.network.name === "bsc") {
      schemaRegistryAddress = BAS_SCHEMA_REGISTRY_ADDRESSES.bscMainnet;
      networkSuffix = "bas";
    } else if (hre.network.name === "bscTestnet") {
      schemaRegistryAddress = BAS_SCHEMA_REGISTRY_ADDRESSES.bscTestnet;
      networkSuffix = "bastest";
    } else {
      console.error(`Unsupported network: ${hre.network.name}. Use 'bsc' or 'bscTestnet'.`);
      process.exit(1);
    }
    ...
    const schemaExists = await verifySchemaExists(schemaRegistry, expectedSchemaUID);
      ...
      const detailsFilePath = path.join(path.dirname(filePath), `${basSchema.name}.deployed.${networkSuffix}.json`);
      const outputData = {
    ...
    console.log(`Deploying schema "${basSchema.name}" to ${hre.network.name} (${schemaRegistryAddress})...`);
    try {
      const tx = await schemaRegistry.register({
        schema: basSchema.schema,
        resolverAddress: resolverAddress,
        revocable: basSchema.revocable,
      });
      ...
      try {
        // Extract transaction hash from BAS transaction response
